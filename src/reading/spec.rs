...
use shared::{ColumnSpec, CollectionSpec, ColumnType, to_column_type};
...
fn match3(buf: &mut Read, column_name: String, column_specs: &mut Vec<ColumnSpec>) {
    ...
    match column_type {
        ColumnType::Set | ColumnType::List | ColumnType::Map => match2(buf, column_type, &mut spec),
        _ => {}
    };
...
fn match1(
    ...
    collection_column_type: ColumnType,
    spec: &mut ColumnSpec,
) {
    match column_type {
        ColumnType::Set => spec.collection_spec = CollectionSpec::Set(collection_column_type),
        ColumnType::List => spec.collection_spec = CollectionSpec::List(collection_column_type),
        ColumnType::Map => {
