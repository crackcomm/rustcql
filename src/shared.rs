...
impl Request {
    pub fn opcode(&self) -> u8 {
        match *self {
            ...
            Request::Query(_, _) |
            Request::PrmQuery(_, _, _) |
            Request::PrmQueryWithNames(_, _, _) |
            Request::PagedQuery(_, _, _, _) |
            Request::PagedPrmQuery(_, _, _, _, _) |
            Request::PagedPrmQueryWithNames(_, _, _, _, _) => 0x07,
            Request::Prepare(_) => 0x09,
            Request::Execute(_, _, _) |
            Request::PagedExecute(_, _, _, _, _) => 0x0A,
